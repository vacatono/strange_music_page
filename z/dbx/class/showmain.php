...
class ShowMain extends sqls
{
	...
	//	ｿﾍﾌｾｸ｡ｺ・	function ShowSomethin($name,$link="main.php",$target="_self"){
		// ｸ｡ｺ・		$sql = "select album.artist,album.title,album.id 
			from players 
	...
	//	ｿﾍﾌｾｸ｡ｺ・	function ShowPerson($name,$link="main.php",$target="_self"){
		// ｸ｡ｺ・		$sql = "select album.artist,album.title,album.id 
			from players 
			left join album on players.album=album.id
	...
	function ShowNew($start=0,$link="main.php",$target="_self"){
		...
		//	ﾇｯﾅﾙ､隍・		$sql ="select left(sales_date,4) as y from album group by y order by y";
	...
	function ShowArtist($name,$link="main.php",$target="_self"){
		// ｸ｡ｺ・		$sql = "select album.artist,album.title,album.id from album 
			where artist='{$name}'
			order by sales_date";
		$data = $this->dbobj->getAssoc($sql);
